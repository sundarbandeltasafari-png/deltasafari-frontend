                    ...
                    <SwiperSlide key={pkg.id || index}>
                        <div className="card h-100 border-0 shadow-sm bg-white rounded-4 overflow-hidden position-relative">
                            ...
                            <div className="position-relative overflow-hidden" style={{ height: '170px' }}>
                                ...
                                <span className="position-absolute top-0 end-0 m-2 badge text-white text-uppercase text-3xs px-2 py-0.5 rounded-2 shadow-xs" style={{ backgroundColor: '#0066cc', zIndex: 10, fontWeight: 500 }}>
                                    {categoryName}
                                </span>

                                {/* Duration & Discount Overlay Banner */}
                                <div className="position-absolute bottom-0 start-0 w-100 bg-dark bg-opacity-75 text-white px-2.5 py-1 text-2xs d-flex align-items-center justify-content-between" style={{ zIndex: 10, fontWeight: 500 }}>
                                    <span><i className="fa-solid fa-clock text-warning me-1"></i>{durationText}</span>
                                    {discountPercent > 0 && (
                                        <span className="badge bg-danger text-white text-3xs px-1.5 py-0.5 rounded-pill" style={{ fontWeight: 500 }}>
                                            {discountPercent}% OFF
                                        </span>
                                    )}
                                </div>
                            ...
                            <div className="card-body p-3 d-flex flex-column justify-content-between">
                                <div>
                                    <h3 className="h6 mb-1 text-dark text-truncate-2" style={{ minHeight: '38px', lineHeight: '1.35', fontFamily: "'Poppins', sans-serif", fontSize: '14px' }}>
                                ...
                                <div className="d-flex align-items-end justify-content-between pt-2 border-top mt-2">
                                    ...
                                    <Link href={detailsUrl} className="primary-btn1 py-1.5 px-2.5 text-2xs" style={{ fontWeight: 500 }}>
                                        <span>Book Now</span>
                                        <span>Book Now</span>
                                    </Link>
                                </div>
