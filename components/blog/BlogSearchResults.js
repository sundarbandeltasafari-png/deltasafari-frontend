    ...
    return (
        <>
            {item && <Link href={"/blogs/" + encodeURI(`${item.slug}-${urlEncode(item.id)}`)}  className="d-flex align-items-center gap-3">
                <img src={process.env.NEXT_PUBLIC_SERVER_URL + item.featured_image} alt={item?.title ? `${item.title} - Delta Safari Blog` : "Delta Safari Travel Article"} className="rounded object-fit-cover" style={{ width: '70px', height: '70px' }} />
                <div>
